...
// MISO (pin 9) connected to PA4 (SSI0R x)
...
#define DC_COMMAND 0
#define DC_DATA 0x01
#define RESET (*((volatile uint32_t *)0x40025008)) // PF1 bit-band
#define RESET_LOW 0
...
void static commonInit(const uint8_t *cmdList)
{
  ...
  GPIO_PORTF_LOCK_R = 0;          // lock GPIO Port F

  GPIO_PORTF_DIR_R |= 0x03;         // PF1 = RESET, PF0 = DC
  GPIO_PORTF_AFSEL_R &= ~0x03;      // disable alternate function
  GPIO_PORTF_DEN_R |= 0x03;         // digital enable
